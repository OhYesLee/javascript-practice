...
게임 함수 (game(player1, player2)): 이 함수는 두 플레이어가 진행하는 한 판을 시뮬레이션합니다. 각 플레이어는 choose() 함수를 통해 가위, 바위, 보 중 하나를 선택하고, 그 결과를 바탕으로 승자를 결정합니다. 비긴 경우는 null을 반환합니다.

1000판 시뮬레이션: 여기서는 1000판의 게임을 시뮬레이션합니다. 각 판마다 두 플레이어를 무작위로 선택하고, 그들 사이에서 한 판을 진행합니다. 비긴 경우에는 현재 반복을 스킵하고 다음 판으로 넘어갑니다. 한 플레이어가 5번 연속으로 이기면, 그 플레이어가 최종 승자가 되고, 게임은 종료됩니다.

승률 출력: 마지막으로 각 플레이어의 승률 (이긴 판 수 / 총 참여한 판 수)을 출력합니다.

따라서, 이 코드는 플레이어들 사이에서 가위바위보 게임을 시뮬레이션하고, 플레이어 각각의 승률과 가장 먼저 5번 연속으로 이긴 플레이어를 결정합니다. 이 코드는 각 판이 독립적이고 플레이어의 선택이 무작위로 이루어진다는 가정 하에 동작합니다.
 */
// Define the players
let players = {
